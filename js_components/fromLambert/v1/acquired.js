class Acquired {
  ...
  components(options) {
    ...
    this.formOrigin = this.setEnviroment(options.environment);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if(iframeDoc) {
        ...
        if(options.hasOwnProperty('confirmParams')){
          params = {confirmParams: JSON.stringify(options.confirmParams), component: _self.componentName};
        }
        this.iframePostMessage(iframeDoc, "submit", params);
      }else{
        reject();
      }
  ...
  handlePostMessage(e) {
    ...
    switch (e.data.event) {
      case "load-response":
        this.handleLoadResponse(e);
        break;
      case "resize-form":
        this.handleResizeForm(e);
        break;
      case "field-blur":
  ...
  handleLoadResponse(e){
    if(e.data.response.component){
      ...
      if(iframeDoc) {
        let params = {component: component};
        if(Object.keys(options).length > 0){
          params = {component: component, options: JSON.stringify(options)};
        }
        this.iframePostMessage(iframeDoc, "element-mount", params);
      }
    }
  }
  ...
  showComponentErrorMessage(element, response) {
    ...
    if(nextElement.className == element.substring(1)){
      ...
      this.iframePostMessage(iframeDoc, "component-error", {component: response.component});
    }
  }
  ...
  iframePostMessage(iframe, event, reqeust){
    iframe.postMessage({event: event, publicKey: this.publicKey, sessionId: this.sessionId, request:reqeust}, this.formOrigin);
  }
}